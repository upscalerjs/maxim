...
def get_params(ckpt_path) -> Params:
...
def get_jax_model(task, ckpt_path, verbose=False) -> tuple[Wrapper, Params]:
    if verbose:
...
# def convert_jax_to_tfjs(saved_model_dir, model, target_path, params, quantization_settings, use_cache=False):
...
#     if os.path.exists(saved_model_dir) is False or use_cache is False:
...
#     )
#     saved_model_conversion.convert_tf_saved_model(saved_model_dir, model_dir, quantization_dtype_map=quantization_dtype_map, skip_op_check=True)
#     update('done')

#     pbar.close()

def get_quantization_dtype_map(quantization_settings: Optional[str]):
    quantization_dtype_map=_parse_quantization_dtype_map(
        float16=None if quantization_settings != 'float16' else True, 
        uint8=None if quantization_settings != 'uint8' else True, 
        uint16=None if quantization_settings != 'uint16' else True, 
